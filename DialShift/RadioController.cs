...
public sealed class RadioController : IDisposable
{
    ...
    public RadioController(Settings settings, Dispatcher dispatcher)
    {
        ...
        timer = new DispatcherTimer(TimeSpan.FromSeconds(1), DispatcherPriority.Background, (_, _) => Tick(), dispatcher);
        timer.Start();
    }
    ...
    private async void Open(Station station)
    {
        ...
        player = session;
        void OnUi(Action action) => dispatcher.BeginInvoke(new Action(() => { if (!disposed && player == session && IsActive) action(); }));
    ...
    private void Tick()
    {
        ...
        // Periodically try the requested station again after a successful fallback.
