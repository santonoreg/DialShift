...
$output = if ($OutputDirectory) { [IO.Path]::GetFullPath($OutputDirectory) } else { Join-Path $root 'artifacts\DialShift-win-x64' }
...
$archive = Join-Path $root 'artifacts\DialShift-0.2.0-win-x64.zip'
